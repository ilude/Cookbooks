...
directory node['wordpress']['dir'] do
  owner (node['apache']['user'] ||= "root")
  group (node['apache']['group'] ||= "root")
  mode "0755"
  action :create
  recursive true
end
...
execute "Set wordpress permissions" do
  command "chown -R #{node['apache']['user'] ||= 'nobody'}:#{node['apache']['group'] ||= 'nogroup'} #{node['wordpress']['dir']}"
end
